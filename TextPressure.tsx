...
const PALETTE = ['#FFFFFF', '#FFE600', '#A78BFA', '#38BDF8', '#EC4899', '#10B981', '#F59E0B'];
...
export const TextPressure: React.FC<TextPressureProps> = ({
  ...
  // Change color only when user interacts / moves mouse over component ("নারা চারা করলে কালার পরিবর্তন হবে")
  const handleInteraction = useCallback(() => {
    setColorIndex((prev) => (prev + 1) % PALETTE.length);
  }, []);

  const activeTextColor = PALETTE[colorIndex] || textColor;
  ...
  }, [fontUrl, activeTextColor, strokeColor]);
        ...
        {chars.map((char, i) => (
          <span
            ...
            style={{
              ...
              color: stroke ? undefined : activeTextColor,
              transition: 'color 0.6s ease'
            }}
          >
            {char === ' ' ? '\u00A0' : char}
